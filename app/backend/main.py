...
def list_routes():
    with get_connection() as conn:
        rows = conn.execute(
            """
            SELECT r.id, r.route_type, r.title, r.service_notes, COUNT(DISTINCT t.id) AS trip_count
            FROM routes r
...
def route_detail(route_id: str):
    with get_connection() as conn:
        ...
        combos = conn.execute(
            ...
            FROM trips
            WHERE route_id = ?
            GROUP BY service_day, direction
            ORDER BY service_day, direction
            """,
            (route_id,),
        ).fetchall()
...
def route_trips(route_id: str, service_day: str, direction: str):
    with get_connection() as conn:
        trips = conn.execute(
            ...
            FROM trips
            WHERE route_id = ? AND service_day = ? AND direction = ?
            ORDER BY trip_index
            """,
    ...
    for trip in trips:
        ...
        response.append({
            **dict(trip),
            "stop_times": [
                {**dict(st), "is_request_stop": bool(st["is_request_stop"])} for st in stop_times
            ],
        })
    return response
...
# ── Frontend (serve SPA) ──────────────────────────────────────────────────────
# Note: These are placed at the absolute bottom so they do not catch API routes.
@app.get("/")
def index():
